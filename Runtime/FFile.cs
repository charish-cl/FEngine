...
namespace FEngine
{
    public static class FFile
    {
        ...
        public struct UnityFileInfo
        {
            ...
            public string Resourcepath //Resources路径,包含name
            {
                get
                {
                    ...
                    {
                        resourcepath = FFile.GetResoucesPath(assetpath,IsDir);
                    }
                    return resourcepath;
                }
            }
            ...
            string path)
        {
            ...
            foreach (var fileInfo in files)
            {
                ...
                var assetpath = GetRelativePath(fileInfo.DirectoryName) + "/" +
                                fileInfo.Name;
                var go = FResources.LoadAssetAtPath(assetpath);
                UnityFileInfo unityFileInfo = new UnityFileInfo()
                {
                    name = go.name,
                    assetpath = assetpath,
        ...
        static void Test2(List<FFile.UnityFileInfo> unityFileInfo)
        {
        ...
        public static void GetAllPaths(List<FFile.UnityFileInfo> unityFileInfo, ref Dictionary<string, string> paths)
        {
            if ( unityFileInfo == null ) return;
            foreach ( var fileInfo in unityFileInfo )
            {
